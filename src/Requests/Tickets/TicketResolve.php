<?php

namespace TeamNiftyGmbH\ResellerInterface\Requests\Tickets;

use Saloon\Contracts\Body\HasBody;
use Saloon\Enums\Method;
use Saloon\Http\Request;
use Saloon\Http\Response;
use Saloon\Traits\Body\HasFormBody;
use TeamNiftyGmbH\ResellerInterface\Dto\Ticket;

/**
 * post_ticket_resolve
 *
 * Schließt ein bestehendes Service-Ticket ab<br /><br />Benötigte Rechte:<br />**Ticketanfragen
 * einsehen** (api.ticket.view)<br /><br /><a target="_blank" href="/core/api#ticket/resolve">In
 * Reseller-Interface öffnen</a>
 */
class TicketResolve extends Request implements HasBody
{
    use HasFormBody;

    protected Method $method = Method::POST;

    /**
     * @param  int  $ticketId  Service-Ticket-ID
     * @param  string  $ticketPassword  Ticket-Passwort
     * @param  bool  $resolved  Ist das Service-Ticket gelöst?
     * @param  null|string  $message  Abschließendes Feedback zur Bearbeitung des Service-Tickets (optional)
     */
    public function __construct(
        protected int $ticketId,
        protected string $ticketPassword,
        protected bool $resolved,
        protected ?string $message = null,
    ) {}

    public function createDtoFromResponse(Response $response): mixed
    {
        return Ticket::from($response->json() ?? []);
    }

    public function defaultBody(): array
    {
        return array_filter([
            'ticketID' => $this->ticketId,
            'ticketPassword' => $this->ticketPassword,
            'resolved' => $this->resolved,
            'message' => $this->message,
        ]);
    }

    public function resolveEndpoint(): string
    {
        return '/ticket/resolve';
    }
}
